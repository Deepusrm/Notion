...
exports = {
    onAppInstallHandler: async function(args){
        ...
        try {
            ...
            const dbId = JSON.parse(response.response).id;
            console.log(dbId);

            await $db.update(`notion_app_iparams`,"set",{"page_id":pageId,"database_id":dbId},{setIf:"exist"});
    ...
    createNote: async function (args) {
        const parentBlock = payloadUtils.parentBlock(args.data.ticket_id, args.data.product, args.data.database_id);
